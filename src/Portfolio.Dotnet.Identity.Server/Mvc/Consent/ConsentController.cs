        ...
        ILogger<ConsentController> logger) : Controller
    {
        ...
        public async Task<IActionResult> Index(ConsentInputModel model)
        {
            ...
            if (result.IsRedirect)
            {
                ...
                if (context?.IsNativeClient() == true)
                {
                    ...
                    return this.LoadingPage("Redirect", result.RedirectUri ?? string.Empty);
                }

                return Redirect(result.RedirectUri ?? string.Empty);
            }
        ...
        private async Task<ProcessConsentResult> ProcessConsent(ConsentInputModel model)
        {
            ...
            else
            {
                ...
                result.ViewModel = await BuildViewModelAsync(model.ReturnUrl ?? string.Empty, model);
            }
